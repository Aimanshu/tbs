      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="row">
                                ...
                                @else
                                  @php $newoptions = json_decode($option->question->options);@endphp
                                    @foreach($newoptions as $newoption)
                                      <input type="{{$option->question->answer_type == 1 ?  'checkbox' : 'radio'}}"   value="{{$newoption->value}}"   style="position: unset;opacity: 4;" disabled {{ $newoption->value == $option->answer ? 'checked' : '' }}/>
                                    {{$newoption->lable}}<br>
                                  @endforeach
                                @endif 
                              </div>
              ...
              <center><a href="{{route('trader_send_proposal',$category->id)}}"><button type="button" class="btn btn-warning waves-effect m-r-20">Send Proposal</button><a/></center>
            </div>
          </div>
